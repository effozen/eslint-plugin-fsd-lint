...
import { extractLayerFromPath, extractLayerFromImportPath, extractSliceFromPath, isTestFile, normalizePath, isRelativePath } from '../utils/path-utils.js';
    ...
    schema: [
      {
        ...
        properties: {
          ...
          typesOnly: {
            type: "boolean",
            description: "If true, allow cross-slice imports for TypeScript type imports"
          },
          excludeLayers: {
            type: "array",
            items: { type: "string" },
            description: "Layers to exclude from this rule (shared is excluded by default)"
          },
          featuresOnly: {
            type: "boolean",
            description: "If true, only apply the rule to features layer (legacy behavior)"
          }
        },
        additionalProperties: false
      }
  ...
  create(context) {
    ...
    return {
      ImportDeclaration(node) {
        ...
        // Check if this is a type-only import (TypeScript feature)
        if (config.typesOnly && node.importKind === 'type') {
          return; // Allow type-only imports between slices if configured
        }
